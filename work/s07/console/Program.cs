...
namespace menu
{
    class Program
    {
        ...
        {
            Console.Write("Enter the number of account you want to show: ");
        ...
        {
            Console.Clear();
            String input = CaptureInput().ToString();
            var accounts = ReadAccounts();
            var table = new ConsoleTable("Number", "Balance", "Label", "Owner");
            foreach (Account account in accounts)
            {
                if(account.Number.ToString()==input)
                table.AddRow(account.Number, account.Balance, account.Label, account.Owner);
            }
            table.Write(Format.Alternative);
            Console.WriteLine();
            ...
            public static HashSet<Type> NumericTypes = new HashSet<Type>
        {
            ...
            public void Write(Format format = Program.Format.Default)
            {
                switch (format)
                {
                    case Program.Format.Default:
                        Options.OutputTo.WriteLine(ToString());
                        break;
                    case Program.Format.MarkDown:
                        Options.OutputTo.WriteLine(ToMarkDownString());
                        break;
                    case Program.Format.Alternative:
                        Options.OutputTo.WriteLine(ToStringAlternative());
                        break;
                    case Program.Format.Minimal:
